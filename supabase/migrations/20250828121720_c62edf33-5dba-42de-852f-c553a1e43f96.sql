...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_order_comments_order_id') THEN
    CREATE INDEX idx_order_comments_order_id ON public.order_comments(order_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_documents_order_id') THEN
    CREATE INDEX idx_documents_order_id ON public.documents(order_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_employee_notes_order_id') THEN
    CREATE INDEX idx_employee_notes_order_id ON public.employee_notes(order_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_diamond_movements_parcel_id') THEN
    CREATE INDEX idx_diamond_movements_parcel_id ON public.diamond_movements(parcel_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_diamond_movements_created_at') THEN
    CREATE INDEX idx_diamond_movements_created_at ON public.diamond_movements(created_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_shipping_entries_order_id') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_diamond_parcels_code') THEN
...
-- Seed customers (no auth dependency)
...
WHERE NOT EXISTS (SELECT 1 FROM public.orders WHERE customer_id = c.id)
LIMIT 5;
